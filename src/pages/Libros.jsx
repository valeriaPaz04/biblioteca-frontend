...
export default function Libros() {
  ...
  const handleSearch = (term) => {
    ...
    if (sortOrder) {
      filtered = sortLibros(filtered, sortOrder);
    }
  ...
  const handleSort = (order) => {
    setSortOrder(order);
    const sorted = sortLibros(filteredLibros, order);
    setFilteredLibros(sorted);
  };

  const sortLibros = (list, order) => {
    return [...list].sort((a, b) => {
      const nameA = `${a.autor.nombre} ${a.autor.apellido}`.toLowerCase();
      const nameB = `${b.autor.nombre} ${b.autor.apellido}`.toLowerCase();
      if (order === 'AZ') {
  ...
  const visibleLibros = filteredLibros.slice(0, visibleCount);

  const handleShowMore = () => {
    setVisibleCount(filteredLibros.length);
  };
  ...
  const downloadPDF = async () => {
    const itemsPerPage = 1; // Número de libros por página

    // Convertir imágenes a base64
    const librosWithBase64 = await Promise.all(libros.map(async (libro) => ({
      ...libro,
      base64Image: await getBase64Image(libro.foto)
    })));

    const pages = [];
    for (let i = 0; i < librosWithBase64.length; i += itemsPerPage) {
      pages.push(librosWithBase64.slice(i, i + itemsPerPage));
    }

    const doc = new jsPDF('p', 'mm', 'a4');

    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      const pageLibros = pages[pageIndex];
      ...
      tempDiv.innerHTML = `
        ...
        <head>
            ...
            <title>Biblioteca Nexus - Catálogo de Libros</title>
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .book-card { border: 1.5px solid #f42535; background-color: #ffffff; padding: 1.5rem; margin-bottom: 1rem; }
                .card-content { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
                .book-info { display: flex; gap: 1.5rem; align-items: center; }
                ...
                .availability { text-align: right; margin-left: 10px; font-weight: bold; }
            </style>
        </head>
        <body>
            <div class="container">
                ...
                <main class="main-content">
                    <h2 class="directory-title">Catálogo de Libros</h2>
                    ${pageLibros.map(libro => `
                        <div class="book-card">
                            <div class="card-content">
                                <div class="book-info">
                                    <img src="${libro.base64Image || ''}" style="width: 5rem; height: 7rem; object-fit: cover; border: 2px solid #d4d4d4;" />
                                </div>
                            </div>
                            <div class="basic-info">
                                <div class="info-row">
                                    <span class="label">Título</span>
                                    <div class="value">${libro.titulo}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Autor</span>
                                    <div class="value">${libro.autor.nombre} ${libro.autor.apellido || ''}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">ISBN</span>
                                    <div class="value">${libro.isbn}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Año de Publicación</span>
                                    <div class="value">${libro.anioPublicacion}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Género</span>
                                    <div class="value">${libro.genero}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Editorial</span>
                                    <div class="value">${libro.editorial}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Descripción</span>
                                    <div class="value">${libro.descripcion}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Disponible</span>
                                    <div class="value">${libro.disponible ? 'Sí' : 'No'}</div>
                                </div>
                            </div>
                        </div>
                    `).join('')}
  ...
  return (
    <>
      <div className="libros-container">
        <h1 className="main-title">Mundos infinitos en cada página</h1>
        ...
        <div className="sort-container">
           <div>
             ...
             <div className="sort-options">
               <span
                 className={`sort-option ${sortOrder === 'AZ' ? 'active' : ''}`}
                 onClick={() => handleSort('AZ')}
               >
                 Autor AZ
               </span>
               <span
                 className={`sort-option ${sortOrder === 'ZA' ? 'active' : ''}`}
        ...
        {visibleCount < filteredLibros.length && (
          <button onClick={handleShowMore}>VER MÁS LIBROS</button>
        )}
      </div>
      ...
      <section className="cta-section">
        <div className="cta-container">
          <div className="cta-box cta-register">
            <p className="cta-title">¡Descarga nuestro catálogo completo!</p>
            <p className="cta-subtitle">Obtén el listado completo de todos los libros disponibles en<br/> Biblioteca Nexus.</p>
          ...
          <div className="cta-box cta-login">
            <div className="cta-login-content">
              ...
              <p className="cta-login-text">¿Deseas registrar un nuevo libro? Ayúdanos a enriquecer nuestra colección bibliográfica.</p>
            </div>
            <Link to="/register-libro" className="cta-login-button">AÑADIR LIBRO</Link>
          </div>
        </div>
      </section>
    </>
