import React, { useEffect, useState } from 'react';
import './css/User.css';
import { Link } from 'react-router-dom';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';

export default function User() {
  const formatDateTime = (date) => {
    const optionsDate = { day: 'numeric', month: 'long', year: 'numeric' };
    const optionsTime = { hour: 'numeric', minute: 'numeric', hour12: true };
    const formattedDate = date.toLocaleDateString('es-ES', optionsDate);
    const formattedTime = date.toLocaleTimeString('es-ES', optionsTime);
    return `${formattedDate} - ${formattedTime}`;
  };

  const getInitials = (name) => {
    const names = name.split(' ');
    if (names.length > 1) {
      return names[0][0] + names[1][0];
    }
    return names[0][0];
  };

  // Estado para usuarios reales
  const [users, setUsers] = useState([]);
  const [prestamosCounts, setPrestamosCounts] = useState({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    const fetchData = async () => {
      try {
        const API_BASE = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
        const [usersRes, prestamosRes] = await Promise.all([
          fetch(`${API_BASE}/api/usuarios`),
          fetch(`${API_BASE}/api/prestamos`)
        ]);
        if (!usersRes.ok) throw new Error("No se pudo obtener usuarios");
        if (!prestamosRes.ok) throw new Error("No se pudo obtener préstamos");

        const usersData = await usersRes.json();
        const prestamosData = await prestamosRes.json();

        // Calcular counts por usuario
        const counts = {};
        prestamosData.forEach(p => {
          if (!p.usuario) return;
          const userId = p.usuario._id || p.usuario;
          if (!counts[userId]) counts[userId] = { activos: 0, devueltos: 0 };
          if (p.estado === 'prestado') counts[userId].activos++;
          else if (p.estado === 'devuelto') counts[userId].devueltos++;
        });
        setPrestamosCounts(counts);
        setUsers(usersData);
      } catch (err) {
        setError(`Error al cargar datos: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
    fetchData();
  }, []);

  const downloadPDF = async () => {
    const itemsPerPage = 2; // Número de usuarios por página
    const pages = [];
    for (let i = 0; i < users.length; i += itemsPerPage) {
      pages.push(users.slice(i, i + itemsPerPage));
    }

    const doc = new jsPDF('p', 'mm', 'a4');

    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      const pageUsers = pages[pageIndex];
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = `
        <!DOCTYPE html>
        <html lang="es">
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Biblioteca Nexus - Directorio de Miembros</title>
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                body { background-color: #141414; color: #000000; font-family: 'Times New Roman', Times, serif; min-height: 100vh; padding: 2rem; }
                .container { max-width: 64rem; margin: 0 auto; }
                .header { margin-bottom: 2rem; text-align: center; background-color: #141414; padding-top: 50px; padding-bottom: 30px; }
                .logo-container { display: flex; align-items: center; justify-content: center; gap: 3rem; }
                .book-icon img { width: 250px; }
                .library-title { font-size: 2.6rem; color: #ffffff; font-family: Georgia, serif; font-weight: normal; }
                .main-content { background-color: #ffffff; padding: 2rem; border: 20px solid #000000; }
                .directory-title { font-size: 2rem; font-weight: bold; color: #f42535; text-align: center; margin-bottom: 2rem; }
                .member-card { border: 1.5px solid #f42535; background-color: #ffffff; padding: 1.5rem; margin-bottom: 1rem; }
                .card-content { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
                .member-info { display: flex; gap: 1.5rem; align-items: center; }
                .avatar { width: 5rem; height: 5rem; border-radius: 50%; border: 2px solid #d4d4d4; background-color: white; display: flex; align-items: center; justify-content: center; font-size: 2.1rem; font-weight: bold; color: #000000; flex-shrink: 0; }
                .basic-info { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; width: 100%; }
                .info-row { margin-bottom: 1rem; }
                .label { font-weight: bold; color: #000000; display: block; }
                .value { color: #000000; margin-top: 3px; }
                .stats { text-align: right; margin-left: 10px}
                .stats-headers { display: flex; gap: 4rem; font-size: 0.875rem; font-weight: bold; color: #000000; margin-bottom: 0.25rem; }
                .stats-numbers { display: flex; gap: 10rem; width: 100%; justify-content: center; font-size: 2.5rem; font-weight: bold; color: #000000; }
            </style>
        </head>
        <body>
            <div class="container">
                <header class="header">
                    <div class="logo-container">
                        <div class="book-icon">
                            <img src="/src/assets/logoBiblioteca.png" alt="Logo de la Biblioteca"/>
                        </div>
                        <h1 class="library-title">Biblioteca Nexus</h1>
                    </div>
                </header>
                <main class="main-content">
                    <h2 class="directory-title">Directorio de Miembros</h2>
                    ${pageUsers.map(user => `
                        <div class="member-card">
                            <div class="card-content">
                                <div class="member-info">
                                    <div class="avatar">${getInitials(user.nombre)}</div>
                                    <div class="stats">
                                        <div class="stats-headers">
                                            <span>Prestamos Activos</span>
                                            <span>Libros Devueltos</span>
                                        </div>
                                        <div class="stats-numbers">
                                            <span>${prestamosCounts[user._id]?.activos ?? 0}</span>
                                            <span>${prestamosCounts[user._id]?.devueltos ?? 0}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="basic-info">
                                <div class="info-row">
                                    <span class="label">Nombre</span>
                                    <div class="value">${user.nombre}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Email</span>
                                    <div class="value">${user.email}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Genero</span>
                                    <div class="value">${user.genero || ''}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Telefono</span>
                                    <div class="value">${user.telefono || ''}</div>
                                </div>
                                <div class="info-row">
                                    <span class="label">Fecha Nacimiento</span>
                                    <div class="value">${user.fechaNacimiento ? new Date(user.fechaNacimiento).toLocaleDateString() : ''}</div>
                                </div>
                            </div>
                        </div>
                    `).join('')}
                </main>
            </div>
        </body>
        </html>
      `;
      tempDiv.style.position = 'absolute';
      tempDiv.style.left = '-9999px';
      tempDiv.style.width = '800px';
      document.body.appendChild(tempDiv);

      const canvas = await html2canvas(tempDiv, { scale: 2 });
      document.body.removeChild(tempDiv);

      const imgData = canvas.toDataURL('image/png');
      const imgWidth = 210;
      const imgHeight = (canvas.height * imgWidth) / canvas.width;

      if (pageIndex > 0) doc.addPage();
      doc.addImage(imgData, 'PNG', 0, 0, imgWidth, imgHeight);
    }

    doc.save('directorio-miembros.pdf');
  };

  return (
    <>
      <div className="user-page-container">
        <h1 className="main-title">Comunidad de lectores</h1>
        <div className="title-border"></div>

        <div className="readers-grid">
          {loading ? (
            <p>Cargando usuarios...</p>
          ) : error ? (
            <p style={{ color: 'red' }}>{error}</p>
          ) : users.length === 0 ? (
            <p>No hay usuarios registrados.</p>
          ) : (
            users.map(reader => (
              <div key={reader._id} className="reader-card">
                <div className="reader-header">
                  <div className="reader-profile-pic-container">
                    {getInitials(reader.nombre)}
                  </div>
                  <div className="reader-basic-info">
                    <h2 className="reader-name">{reader.nombre}</h2>
                    <p className="reader-email">{reader.email}</p>
                  </div>
                </div>

                <div className="reader-stats">
                  <div className="reader-stat-box">
                    <span className="reader-stat-number active-loans">{prestamosCounts[reader._id]?.activos ?? 0}</span>
                    <span className="reader-stat-label">Préstamos Activos</span>
                  </div>
                  <div className="reader-stat-box">
                    <span className="reader-stat-number">{prestamosCounts[reader._id]?.devueltos ?? 0}</span>
                    <span className="reader-stat-label">Libros Devueltos</span>
                  </div>
                </div>

                <div className="reader-separator"></div>

                <div className="reader-membership-info">
                  <p className="reader-membership-item">
                    <i className="fi fi-rr-calendar-lines"></i> Miembro desde: {reader.fechaRegistro ? new Date(reader.fechaRegistro).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' }) : 'Sin fecha'}
                  </p>
                  <p className="reader-membership-item">
                    <i className="fi fi-rr-clock-three"></i> Último acceso: {reader.ultimoAcceso ? new Date(reader.ultimoAcceso).toLocaleString('es-ES') : 'Sin acceso'}
                  </p>
                </div>
              </div>
            ))
          )}
        </div>
      </div>

      <section className="cta-section">
        <div className="cta-container">
          <div className="cta-box cta-register">
            <p className="cta-title">¡Descarga el directorio de miembros!</p>
            <p className="cta-subtitle">Obtén la lista completa de nuestra comunidad de lectores.</p>
            <div className="cta-form">
              <button type="button" onClick={downloadPDF}>DESCARGAR CATÁLOGO</button>
            </div>
          </div>
          <div className="cta-box cta-login">
            <div className="cta-login-content">
              <svg
                className="cta-logo"
                viewBox="0 0 120 53" 
                width="120"
                height="53"
                xmlns="http://www.w3.org/2000/svg" 
              >
                <path 
                  className="svg-path-color"
                  d="M81.167 11.151l1.293 2.909.645-3.07 3.071-.323-2.746-1.616.646-2.908-2.264 1.939-2.583-1.455 1.13 2.747-2.261 2.101zM98.456 28.117l2.101-2.424 2.748 1.293-1.455-2.585 1.938-2.424-2.908.647-1.616-2.586-.325 3.07-3.068.646 2.908 1.293zM57.738 7.597l2.585-1.939 2.424 1.939-.97-3.07 2.584-1.777h-3.068l-.97-2.909-.969 2.909h-3.07l2.424 1.777zM37.379 10.99l.646 3.07 1.293-2.909 3.07.324-2.424-2.101 1.293-2.747-2.586 1.455-2.423-1.939.646 2.908-2.585 1.616zM17.182 26.986l2.746-1.293 2.101 2.424-.323-3.07 2.747-1.293-2.909-.646-.323-3.07-1.616 2.586-3.07-.647 2.101 2.424zM119.784 49.932l-1.454-2.425H62.585c0-1.938 2.101-1.938 3.231-1.938h51.543l-1.453-2.424H67.109c-1.293 0-3.555.162-4.201.322.646-2.262 4.361-2.424 9.533-2.424h42.172l-1.454-2.423H73.411c-4.04 0-8.079.161-10.181 1.614 1.938-2.745 5.334-3.555 13.572-3.555h35.225l-1.453-2.424H76.966C64.848 34.257 60 37.328 60 47.021c0-9.693-4.847-12.766-16.966-12.766H9.426L7.972 36.68h35.062c8.24 0 11.634.81 13.573 3.555-2.101-1.453-5.979-1.614-10.18-1.614H6.841l-1.454 2.423h42.171c5.333 0 8.887.162 9.533 2.424-.646-.16-2.908-.322-4.201-.322H4.094l-1.454 2.42h51.543c1.131 0 3.232 0 3.232 1.939H1.67L.216 49.93h57.36v1.454h5.009V49.93h57.199v.002z"
                />
              </svg>
              <p className="cta-login-text">¿Deseas registrar un nuevo miembro? Amplía nuestra comunidad de lectores añadiendo nuevos usuarios.</p>
            </div>
            <Link to="/register-user" className="cta-login-button">ANADIR USUARIO</Link>
          </div>
        </div>
      </section>
    </>
  );
}