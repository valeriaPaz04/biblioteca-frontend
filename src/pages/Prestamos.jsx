...
export default function Prestamos() {
  ...
  const handleDevolver = async (id) => {
    if (!window.confirm('¿Confirmas que deseas devolver este libro?')) return;
    try {
      const API_BASE = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
      const res = await fetch(`${API_BASE}/api/prestamos/devolver/${id}`, {
        method: 'PUT'
      });
      if (res.ok) {
  ...
  return (
    <div className="page-container">
      <h1 className="main-title">Préstamos de libros</h1>
        ...
        {prestamos.length === 0 ? (
          ...
          prestamos.map((p) => (
            <div key={p._id} className="prestamo-card">
              ...
              {p.estado !== 'devuelto' && (
                <button className="devolver-button" onClick={() => handleDevolver(p._id)}>DEVOLVER LIBRO</button>
              )}
            </div>
          ))
        )}
      ...
      <section className="cta-section">
        <div className="cta-container">
          <div className="cta-box cta-register">
            <p className="cta-title"> ¡Descarga tu historial de préstamos!</p>
            <p className="cta-subtitle">Obtén un reporte completo de todos tus préstamos activos y anteriores</p>
            <div className="cta-form">
              <button type="button" onClick={downloadPDF}>DESCARGAR HISTORIAL</button>
            </div>
          </div>
          <div className="cta-box cta-login">
            <div className="cta-login-content">
              ...
              <p className="cta-login-text">¿Deseas gestionar todos los préstamos? Accede aquí para supervisar todos los préstamos de la biblioteca.</p>
            </div>
            <Link to="/register-prestamo" className="cta-login-button">GESTIONAR PRÉSTAMOS</Link>
          </div>
        </div>
      </section>
